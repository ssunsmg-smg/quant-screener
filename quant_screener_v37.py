...
#      "stop_loss_pct":       5.0,            ← 손절 기준(%) — 보유종목 합산수익률 이하시 전량매도
#      "take_profit_min_pct": 1.0,            ← 익절 시작 기준(%)
#      "take_profit_max_pct": 20.0,           ← 익절 종료 기준(%) — 합산수익률 1~20% 범위면 전량매도
...
class KISAutoTrader:
    ...
    def execute_signals(self, df_signals: pd.DataFrame, total_capital: int) -> list:
        """
        스크리닝 매매 시그널 자동 실행 — 포트폴리오(바스켓) 단위 매매

        ── 핵심 로직: 개별 종목이 아니라 "보유 종목 전체 묶음"으로 판단 ──
        ① 보유 중인 모든 종목의 (현재가×수량) 합계를 구해서
           포트폴리오 전체 수익률 = (현재평가액합 - 매수원금합) / 매수원금합 을 계산
        ② 그 합산 수익률이
             -stop_loss_pct 이하        → 보유 종목 전부 동시 매도(손절)
             +tp_min ~ +tp_max 범위     → 보유 종목 전부 동시 매도(익절)
             그 사이                    → 전부 그대로 보유 (매도 안 함)
        ③ 전부 매도된 날에만, 회수된 총액을 재투자 풀에 반영하고 신규 매수 진행
           (포지션이 남아있는 동안은 신규 매수를 하지 않음 — 바스켓이 통째로 빌 때만 교체)

        ── 재투자 풀(계좌 단위 복리) ──
        예) base_invest_amount = 1,000만원, 매수 시그널 5종목
            1일차: 1,000만원 ÷ 5 = 종목당 200만원 매수
            n일차: 5종목 합산 수익률이 +10% 도달 → 5종목 전부 매도, 회수금 1,100만원
            다음날: 매수 시그널이 4종목이면 1,100만원 ÷ 4 = 종목당 275만원 매수
            ... (포트폴리오 전체가 매도될 때마다 그 회수금이 다음 매수 예산으로 누적)
        """
        if not self._is_configured():
            print("  ⚠ KIS 앱키 미설정 → 자동매매 중단")
            return []

        stop      = self.cfg.get("stop_loss_pct", 5.0) / 100
        tp_min    = self.cfg.get("take_profit_min_pct", 1.0) / 100
        tp_max    = self.cfg.get("take_profit_max_pct", 20.0) / 100
        ...
        # ① 포트폴리오(바스켓) 전체 합산 수익률 계산
        recovered_today = 0.0
        basket_sold = False

        if self.positions:
            print(f"\n  [KIS] 포트폴리오 점검 (보유 {len(self.positions)}종목, 바스켓 단위 판단)...")
            print(f"     손절: -{stop*100:.1f}% | 익절: +{tp_min*100:.1f}%~+{tp_max*100:.1f}% "
                  f"(보유 종목 전체 합산 수익률 기준)")

            cost_sum = 0.0
            value_sum = 0.0
            quotes = {}   # code -> 현재가 (나중에 매도할 때 재사용, 중복 조회 방지)

            for code, pos in self.positions.items():
                cur = self.get_current_price(code)
                time.sleep(api_delay)
                if cur <= 0:
                    # 시세 조회 실패 종목은 매수원가로 대체(포트폴리오 계산에서 누락되지 않게)
                    cur = pos["avg_price"]
                quotes[code] = cur
                cost_sum  += pos["avg_price"] * pos["qty"]
                value_sum += cur * pos["qty"]

            basket_ret = (value_sum - cost_sum) / cost_sum if cost_sum > 0 else 0.0
            print(f"     매수원금합: {cost_sum:,.0f}원 | 현재평가합: {value_sum:,.0f}원 | "
                  f"포트폴리오 수익률: {basket_ret*100:+.2f}%")

            sell_reason = None
            if basket_ret <= -stop:
                sell_reason = f"바스켓 손절 {basket_ret*100:+.2f}%"
            elif tp_min <= basket_ret <= tp_max:
                sell_reason = f"바스켓 익절 {basket_ret*100:+.2f}%"

            if sell_reason:
                print(f"  {'🔴' if basket_ret <= -stop else '🟢'} {sell_reason} "
                      f"→ 보유 {len(self.positions)}종목 전체 동시 매도")
                for code, pos in list(self.positions.items()):
                    r = self.place_order(code, "SELL", pos["qty"], reason=sell_reason)
                    if r.get("success"):
                        sell_value = pos["qty"] * quotes.get(code, pos["avg_price"])
                        recovered_today += sell_value
                        action = "STOP_LOSS" if basket_ret <= -stop else "TAKE_PROFIT"
                        executed.append({"action": action, "code": code,
                                         "ret_pct": round(basket_ret * 100, 1),
                                         "recovered_won": round(sell_value)})
                    time.sleep(api_delay)
                basket_sold = True
            else:
                print(f"     판단: 범위 밖 → 전체 보유 유지 (매도 없음)")

        # ── 재투자 풀 갱신 ──
        # 오늘 바스켓 전체가 매도됐으면: 회수금을 풀에 반영.
        # 풀이 아예 None(최초 실행)이면 base_amt로 시작.
        if basket_sold and recovered_today > 0:
            self._reinvest_pool = (self._reinvest_pool or 0.0) + recovered_today
            self._save_reinvest_pool(
                self._reinvest_pool,
                note=f"바스켓 매도 회수 {recovered_today:,.0f}원 누적"
            )
            print(f"  💰 바스켓 매도 회수: {recovered_today:,.0f}원 → "
                  f"재투자 풀: {self._reinvest_pool:,.0f}원")
        elif self._reinvest_pool is None:
            self._reinvest_pool = base_amt

        # ② 신규 매수 — 포지션이 남아있으면(바스켓이 아직 안 비었으면) 매수하지 않음
        if self.positions and not basket_sold:
            print(f"  [KIS] 보유 포지션 유지 중 → 신규 매수 스킵")
            return executed
        ...
        n_buy_targets = len(buy_df) if len(buy_df) > 0 else 1
        per_stock_budget = invest_budget / n_buy_targets   # N종목이면 예산을 N분할
        print(f"  [재투자] 종목당 매수 배분: {per_stock_budget:,.0f}원 "
              f"({invest_budget:,.0f}원 ÷ {n_buy_targets}종목)")
...
def build_excel(df_all, df_top, label, args, output_dir=BASE_DIR):
    ...
    # 헤더 색상 그룹
    _ALL_HDR_GROUPS = {
        range(1,8):   "1F3864",   # 기본 (순위~시가총액) — 발행주식수도 이 그룹에 포함
        range(8,19):  "2E75B6",   # 발행주식수 + 거래량 + 기술지표
        range(19,36): "375623",
        range(36,44): "7030A0",
        range(44,50): "4A4A4A",
        range(50,67): "833C00",
    ...
    _all_fmt = {
        6:"#,##0", 7:"#,##0", 8:"#,##0",
        9:"#,##0.00", 10:"#,##0.00", 11:"#,##0.0",
        12:"#,##0.0", 13:"#,##0.0",
        14:"#,##0.0", 15:"#,##0.0", 16:"#,##0.0",
        17:"#,##0.0", 18:"#,##0.0", 19:"#,##0.0", 20:"#,##0.0",
        21:"#,##0.0", 22:"#,##0.0", 23:"#,##0.0", 24:"#,##0.00",
        25:"#,##0", 26:"#,##0.0", 27:"#,##0.0",
        28:"#,##0", 29:"#,##0.0", 30:"#,##0.0",
        31:"#,##0", 32:"#,##0.0",
        33:"#,##0.0", 34:"#,##0.0", 35:"#,##0.0",
        36:"#,##0.0", 37:"#,##0.0", 38:"#,##0.0",
        39:"#,##0.0", 40:"#,##0.0", 41:"#,##0.0", 42:"#,##0.0",
        43:"#,##0.0",
        44:"#,##0", 45:"#,##0", 46:"#,##0", 47:"#,##0", 48:"#,##0.0",
        50:"#,##0.0",
...
def interactive_menu():
    ...
    if mode == "5":
        ...
        print("  → stop_loss_pct: 손절 기준(%) — 기본 5.0 (합산수익률 이하면 전종목 동시매도)")
        print("  → take_profit_min_pct ~ take_profit_max_pct: 익절 범위(%) — 기본 1.0~20.0")
